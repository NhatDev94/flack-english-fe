...
const BASE_URL = environment.customerBaseUrl;

@Injectable()
export class CustomerBaseService extends GenericService {
  ...
  protected downloadFile(url: string, body?: object): Observable<any> {
    return this.http.get(BASE_URL + url, {
      params: this.getParams(body),
      responseType: 'blob'
    });
